...
exports.cmsGet = (req, res, next) => {
  CMSField.find({})
    .exec((err, cmsFields) => {
      ...
      const { imagesUploaded, imagesNotUploaded, error } = req.query;
...
exports.cmsUploadImages = (req, res) => {
  if (req.fileValidationError) {
    return res.redirect(`/admin/cms?error=${req.fileValidationError}`);
  }
  if (!req.files) {
...
exports.membershipsMultiplePost = (req, res, next) => {
  ...
  }).on('end', () => res.redirect('/admin/kuulumised'));
};
